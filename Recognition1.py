...
from database import get_info, get_data, lidar_coord, distance, post_event, is_point_inside_polygon
...
def check_data_range_x(data_list, threshold):
    # 确保数据点至少有3个
    if len(data_list) < 3:
        return 0
    ...
    # 计算数据点之间的差值，并统计在范围内的个数
    count_within_range = 0
    for i in range(len(data_list)):
        for j in range(i + 1, len(data_list)):
            if abs(data_list[i] - data_list[j]) <= threshold:
                count_within_range += 1

    # 判断是否有至少3个数据在范围内
    return 1 if count_within_range >= 3 else 0

def check_data_range_y(data_list, threshold):
    # 确保数据点至少有3个
    if len(data_list) < 3:
        return 0
...
def check_x(car_info):
    num_to_x = {}
    has = 0
    # 遍历cars_info列表中的每个字典
    for car in cars_info:
        num = car['num']
        x_coord = car['xyz'][0]  # 提取xyz元组中的x坐标

        # 检查num是否已经在num_to_x字典中，如果没有，添加一个空列表
        if num not in num_to_x:
            num_to_x[num] = []

        # 将具有相同num值的x坐标信息添加到对应的列表中
        num_to_x[num].append(x_coord)

    # 输出结果
    for num, x_list in num_to_x.items():
        print(f"num {num} 对应的 x 坐标信息：{x_list}")
        result = check_data_range_x(x_list, 3)
        print(result)
        if result == 1:
            has = 1
    return 1 if has == 1 else 0


def check_y(car_info):
    num_to_y = {}
    has = 0
    # 遍历cars_info列表中的每个字典
    for car in cars_info:
        num = car['num']
        y_coord = car['xyz'][1]  # 提取xyz元组中的y坐标

        # 检查num是否已经在num_to_x字典中，如果没有，添加一个空列表
        if num not in num_to_y:
            num_to_y[num] = []

        # 将具有相同num值的x坐标信息添加到对应的列表中
        num_to_y[num].append(y_coord)

    # 输出结果
    for num, y_list in num_to_y.items():
        print(f"num {num} 对应的 y 坐标信息：{y_list}")
        result = check_data_range_y(y_list, 3)
        print(result)
        if result == 1:
            has = 1
    return 1 if has == 1 else 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', type=str, default='img_txt', help='source')
    parser.add_argument('--lighten_image_path', type=str, default='img_txt/lighten', help='source')
    parser.add_argument('--img_size', type=int, default=384, help='inference size (pixels)')
    parser.add_argument('--output', type=str, default='plate_imgs', help='source')
    ...
    print(opt)
    park_area = []
    ex0, ey0, ez0 = (-2292015.1395, 5002512.6552, 3214449.1065)  # 小车的地心地固坐标
    ...
    num2=0
    num3=0
    for i in range(1,7):  # 对每张图进行处理 zip将event car信息打包
        # data1(time时间,pic照片编码,status雷达文件) data2(time,lon,lat,h,head_angle)
        new_lidar=f"7.27/exp2/pointcloud_{i}.txt"
        lidar_list.append(new_lidar)
        pic = f"7.27/exp2/image_{i}.jpg"
        print("图片路径 ：", pic)
        mod_img = ps_shadow_highlight_adjust_and_save_img(pic)#提亮处理
        mod_pic_path = os.path.join(lighten_path, pic)
        ...
        for j, point in enumerate(point_list):  # point是图中每辆车的点云坐标
            ...
            llh = car_geo(5.88746977, 1.02342606, -0.34505868, ex0, ey0, ez0, -87.830902, 30.460104929, 114.615962345)
            print("llh:", llh)
            lat = llh[0]  # 纬度
            lon = llh[1]  # 经度
            ...
            # 车牌号临时变量plate_t
            if plt_boxes and j < len(plt_list):
                plate_t = plt_list[j].get('plate_no', '未检测到车牌号')
            else:
        ...
        save_img_path = os.path.join(save_path, pic)
        time_e = time.time()
    ...
    final_result=False
    if 1:#经纬度判断结果
      num1=check_x(car_info)#检查x变化范围
    if num1:
      num2=check_y(car_info)#检查y变化范围
    if num2:
        final_result=True#改变违停结果
    print(final_result)
        # print(result)
        # if result == 1:
        #     has=1
    ...
    # 判断违停并回传
    # cursor1 = db.cursor()  # 创建游标对象
    # for info in cars_info:
    #     if info['amount'] == 1:  # 该车牌只有一张
    #         if info['whe_in'] > 0:
    #             sign = "违停车牌号为:" + info['plate']
    #             pic_id = info['id']
    #             sql = "UPDATE event set car_id = '{}' WHERE id = '{}' ".format(sign, pic_id)
    #             cursor1.execute(sql)
    #             #db.commit()
